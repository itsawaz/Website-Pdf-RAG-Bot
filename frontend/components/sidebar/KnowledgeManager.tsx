...
import { FileText, Globe, Database, Trash2, Eye, EyeOff, CheckSquare, Square, X, Brain, Search, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        >
          <X className="w-4 h-4" />
        </button>      </div>      {/* Quick Delete Section */}
      {stats && stats.total_chunks > 0 && (
        <div className="mx-4 p-3 bg-destructive/10 border border-destructive/20 rounded-lg flex-shrink-0">
          <div className="flex items-center gap-2 mb-2">
            <AlertTriangle className="w-4 h-4 text-destructive" />
            <h3 className="text-sm font-medium text-destructive">Quick Delete</h3>
          </div>
          <div className="space-y-1 max-h-32 overflow-y-auto scrollbar-hide">
            <button
              onClick={onClearKnowledgeBase}
              className="w-full px-2 py-1.5 text-xs bg-destructive/20 hover:bg-destructive/30 text-destructive rounded transition-colors flex items-center gap-2"
            >
              <Trash2 className="w-3 h-3" />
              Clear All Knowledge
            </button>
            {Object.keys(stats.sources).map((sourceType) => (
              <button
                key={sourceType}
                onClick={() => onDeleteBySource(sourceType)}
                className="w-full px-2 py-1.5 text-xs bg-destructive/10 hover:bg-destructive/20 text-destructive rounded transition-colors flex items-center justify-between"
              >
                <div className="flex items-center gap-2">
                  <Trash2 className="w-3 h-3" />
                  <span>Delete {sourceType}</span>
                </div>
                <span className="text-xs">({stats.sources[sourceType]})</span>
              </button>
            ))}
          </div>
        </div>
      )}      {/* Search and View Controls */}
      <div className="mx-4 p-3 border-b border-border space-y-2 flex-shrink-0">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-7 pr-3 py-1.5 bg-secondary border border-border rounded focus:outline-none focus:ring-1 focus:ring-primary text-xs"
          />
        </div>
